...
public class WhatsAppCampaignController implements WhatsAppCampaignControllerApi {
    ...
    @ExceptionHandler(WhatsAppTemplateScopeMismatchException.class)
    public ResponseEntity<ErrorResponse> handleScopeMismatch(WhatsAppTemplateScopeMismatchException ex, WebRequest request) {
        log.warn("WhatsApp template scope mismatch: {}", ex.getMessage());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(ErrorResponse.of(HttpStatus.BAD_REQUEST, "Bad Request", ex.getMessage(), request));
    }
}
