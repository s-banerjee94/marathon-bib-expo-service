    ...
    @Operation(
            ...
            description = """
                    ...
                    *n* fills the Twilio template variable `{{n}}` — using the same `#{fieldName}` \
                    placeholders as SMS templates. `senderScope` says which Twilio account the Content \
                    SID lives under (`DEFAULT` = application account, `ORGANIZATION` = the organization's \
                    own). An event can have a maximum of 20 WhatsApp templates. Template creation and \
