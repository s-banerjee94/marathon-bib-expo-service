...
public class EventStatsServiceImpl implements EventStatsService {
    ...
    public void onBibCollected(ParticipantDDB p, List<String> goodiesDistributed) {
        runSafely(p.getEventId(), "onBibCollected", () -> {
            ...
            d.race(p.getRaceId(), p.getRaceName(), 0, +1);
            d.category(p.getCategoryId(), p.getCategoryName(), 0, +1);
    ...
    public void onBibUndone(ParticipantDDB before) {
        runSafely(before.getEventId(), "onBibUndone", () -> {
            DeltaBuilder d = new DeltaBuilder();
            d.simple(KEY_BIB_COLLECTED, -1);
            d.race(before.getRaceId(), before.getRaceName(), 0, -1);
            d.category(before.getCategoryId(), before.getCategoryName(), 0, -1);
    ...
    public ParticipantStatisticsResponse reconcile(Long eventId, User currentUser) {
        ...
        ReconcileState state = aggregateParticipants(eventId);
    ...
    private ReconcileState aggregateParticipants(Long eventId) {
    ...
    private static void aggregateOne(ReconcileState s, ParticipantDDB p) {
        s.total++;
        ...
        accumulateRace(s, p, collected);
        accumulateCategory(s, p);
    }
    ...
    private static void accumulateRace(ReconcileState s, ParticipantDDB p, boolean collected) {
    ...
    private static void accumulateCategory(ReconcileState s, ParticipantDDB p) {
    ...
    private static void applyParticipantPresence(DeltaBuilder d, ParticipantDDB p, long sign) {
        ...
        d.race(p.getRaceId(), p.getRaceName(), sign, collectedSign);
        d.category(p.getCategoryId(), p.getCategoryName(), sign, collectedSign);
    ...
    private static final class DeltaBuilder {
        ...
        private final Map<String, String> raceNames = new HashMap<>();
        ...
        void race(String raceId, String raceName, long totalDelta, long collectedDelta) {
            dimension(PREFIX_RACE, raceId, raceName, totalDelta, collectedDelta, raceNames);
        }

        void category(String categoryId, String categoryName, long totalDelta, long collectedDelta) {
            dimension(PREFIX_CATEGORY, categoryId, categoryName, totalDelta, collectedDelta, categoryNames);
        }

        private void dimension(String prefix, String id, String label, long totalDelta, long collectedDelta,
                               Map<String, String> labelStore) {
            if (id == null) return;
            String k = prefix + id;
            if (totalDelta != 0) {
                counts.merge(k, totalDelta, Long::sum);
                if (label != null) labelStore.putIfAbsent(k, label);
            }
            if (collectedDelta != 0) {
                String ck = k + SUFFIX_COLLECTED;
                counts.merge(ck, collectedDelta, Long::sum);
                if (label != null) labelStore.putIfAbsent(ck, label);
            }
        }
        ...
        Map<String, CounterDelta> build() {
            ...
            counts.forEach((k, v) -> {
                if (v == 0L) return;
                result.put(k, new CounterDelta(v, raceNames.get(k), categoryNames.get(k)));
            });
            return result;
        }
    }
