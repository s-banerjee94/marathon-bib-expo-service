package com.timekeeper.bibexpo.importer.service;
...
public interface BatchImportService {
     ...
     */
    BatchJobStatusResponse getJobStatus(Long eventId, Long jobExecutionId, User currentUser);
     ...
     * @param currentUser      caller, whose organization must own the event
     ...
     * @param eventId     event the mapping is being built for
     * @param currentUser caller, whose organization must own the event
     * @return the list of mappable fields with key, label, and required flag
     */
    List<ImportFieldResponse> getImportFields(Long eventId, User currentUser);
}
