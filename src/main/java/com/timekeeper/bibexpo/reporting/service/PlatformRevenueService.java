...
import com.timekeeper.bibexpo.reporting.model.dto.response.PlatformRevenueTrendDto;
...
public class PlatformRevenueService {
    ...
    private final BillingRevenueQuery billingRevenue;
    ...
    public PlatformRevenueResponse buildRevenue(DashboardRange range, int trendBuckets, TrendInterval trendInterval) {
        ...
        BigDecimal earned = billingRevenue.collectedBetween(from, null);
        ...
        if (range != DashboardRange.ALL) {
            ...
            BigDecimal prior = billingRevenue.collectedBetween(
        ...
        return PlatformRevenueResponse.builder()
                .refreshedAt(now)
                .currency(billingRevenue.currency())
                .range(range)
                .earned(earned)
                .deltaPct(deltaPct)
    ...
    private PlatformRevenueTrendDto buildTrend(int buckets, TrendInterval interval) {
        ...
        for (int i = 0; i < starts.size(); i++) {
            ...
            earned.add(billingRevenue.collectedBetween(start, end));
        }
